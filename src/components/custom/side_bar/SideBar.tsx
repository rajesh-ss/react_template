		...
		>
			<div className={`py-3 px-2 h-20 ${false ? 'hidden' : ''}`}>
				<Select onValueChange={onValueChange}>
					<SelectTrigger className={`border-0 border-b-2 border-b-slate-200 dark:border-b-[#323232] dark:text-white `}>
					...
					<SelectContent className="bg-white dark:bg-[#212121] dark:border-0">
						<SelectGroup className="dark:text-white w-full">
							<SelectLabel className="dark:bg-[#232222] bg-[#c5c3c3]">Choose one</SelectLabel>
							<SelectItem value="chat" className="dark:hover:bg-slate-700 cursor-pointer ">
								<span className="flex items-center gap-2">
									<BsChatLeftText className={`w-4 h-4 hover:scale-105 cursor-pointer inline`} />
									<p className="inline text-sm">Chat</p>
								</span>
							</SelectItem>
							<SelectItem value="pdf" className="dark:hover:bg-slate-700 cursor-pointer">
								<span className="flex items-center gap-2">
									<AiOutlineFilePdf className={`w-4 h-4 hover:scale-105 cursor-pointer inline`} />
			...
			{sidePanelViewMode === 'chat' ? (
				<ul
					className={`${
						props?.files === undefined ? 'h-[calc(100vh-3.6rem)]' : 'h-[calc(100vh-3.6rem-5rem)]'
					} overflow-y-auto w-full scroll-bar-small-w scroll-bar-transparent scroll-bar-thumb-rounded scroll-bar-thumb-grey`}
				>
					{props?.qnsAns?.map((eachChat) => (
						<li className=" px-2 py-1 rounded-[0.3rem]  dark:text-white" key={eachChat?.id}>
							<Accordion type="single" collapsible className="w-full  break-all hover:bg-[#dddbdb] dark:hover:bg-[#282727] rounded-xl px-2 py-0">
								<AccordionItem value={eachChat?.id} className="border-0 w-full m-0">
									<AccordionTrigger className="hover:no-underline">
										<span className="flex items-center gap-3 group">
											<ChatBubbleIcon className={`w-4 h-4 hover:scale-105 cursor-pointer group-hover:hidden`} />
											<IoNavigateCircleOutline className="dark:text-[#fff] text-[#000] text-2xl text-right hidden group-hover:block hover:scale-105" />
											<p className="hover:no-underline">{eachChat?.qns?.length > 15 ? eachChat?.qns?.substring(0, 15) + '..' : eachChat?.qns}</p>
										</span>
									</AccordionTrigger>
