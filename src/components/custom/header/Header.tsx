...
import { MoonIcon, SunIcon, DesktopIcon, HamburgerMenuIcon } from '@radix-ui/react-icons';
		...
		<header className="h-[3.6rem] grid grid-cols-12 dark:bg-[#000] ">
			<nav className="col-span-8 flex justify-start items-center h-full px-5 gap-28">
				<div className=" text-[#454343] flex gap-10 items-center ">
					<span className={`p-2 hover:bg-[#dfdbdb] dark:hover:bg-[#353535] rounded-full ${IsPicQuestSideBarOpenState ? 'dark:bg-[#343333] bg-[#dfdbdb]' : ''}`}>
						<HamburgerMenuIcon className={`w-5 h-5 cursor-pointer hover:scale-105 dark:text-white  `} onClick={onClickHamburgerMenu} />
					</span>
					<h1 className="text-[1rem] font-bold dark:text-white logo-font">Cyber GPT</h1>
				</div>
				<ul className="flex gap-2 text-[#646363] dark:text-[#e7e4e4] text-sm ">
					{props?.nav?.map((eachEle) => (
						<NavLink to={eachEle?.link} className={({ isActive }) => (isActive ? `active-nav nav-font` : `inactive-nav`)} key={nanoid()}>
							{eachEle?.name}
						</NavLink>
					))}
				</ul>
				...
				<div className=" flex justify-end gap-10 items-center h-full ">
					<span className=" dark:text-white flex items-center gap-4">
						...
						<span>
							...
							<h4 className="text-sm font-semibold inline mx-1 nav-font">Simplyfi</h4>
						</span>
					</span>
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Button variant="destructive" size={'sm'} className="hover:bg-[#c7c7c7] dark:hover:bg-[#757373] rounded-[.4rem] dark:text-[#fff]">
								{theme === 'dark' ? <MoonIcon className="w-6 h-6 mx-auto " /> : <></>}
								{theme === 'light' ? <SunIcon className="w-6 h-6 mx-auto" /> : <></>}
								{theme === 'system' ? <DesktopIcon className="w-6 h-6 mx-auto" /> : <></>}
							</Button>
						</DropdownMenuTrigger>
						<DropdownMenuContent className="min-w-32 max-w-32 bg-[#fff] dark:bg-[#544d4d] rounded-[0.4rem] " side="bottom" align="center">
							<DropdownMenuRadioGroup value={theme ? theme : ''} onValueChange={setTheme} className="">
								<DropdownMenuRadioItem value="dark" className={`dark:bg-[#d6d5d5] dark:dark:bg-[#5a5858] p-3 w-full`}>
									<MoonIcon className="w-4 h-4 mx-auto text-[#000] dark:text-[#fff]" />
								</DropdownMenuRadioItem>
								<DropdownMenuRadioItem value="light" className={`dark:bg-[#d6d5d5] dark:dark:bg-[#5a5858] p-3 w-full`}>
									<SunIcon className="w-4 h-4 mx-auto text-[#000] dark:text-[#fff]" />
								</DropdownMenuRadioItem>
								<DropdownMenuRadioItem value="system" className={`dark:bg-[#d6d5d5] dark:dark:bg-[#5a5858] p-3 w-full`}>
